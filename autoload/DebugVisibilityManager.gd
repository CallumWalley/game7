...
func encounter_worker_type(node_type: String) -> void:
	"""Mark a worker type as encountered, making it visible in the UI."""
	if node_type not in _encountered_types:
		push_warning("Unknown worker type: %s" % node_type)
		return
	
	if _encountered_types[node_type]:
		return
	
	_encountered_types[node_type] = true
	visibility_changed.emit("worker_type_%s" % node_type, true)
